	...
	public class FarmToolsOfTaskService : IFarmToolsOfTaskService
	{
		...
		public async Task<IEnumerable<FarmToolsOfTaskResponseDTO>> GetAllFarmToolsOfTasksAsync()
		{
            var farmToolsOfTasks = await Task.FromResult(_unitOfWork.FarmToolsOfTaskRepository.Get(
               includeProperties: "FarmTools,UserTask"
           ));
		...
		public async Task<FarmToolsOfTaskResponseDTO> UpdateFarmToolsOfTasksExtendAsync(FarmToolsOfTaskExtendRequestDTO request)
		{
            ...
            {
                throw new KeyNotFoundException($"Không đủ công cụ nông trại cho {farmTools.FarmToolsName}. Có sẵn: {farmTools.FarmToolsQuantity}, Đã yêu cầu: {farmToolsOfTask.FarmToolOfTaskQuantity}");
            }
		...
		public async Task<FarmToolsOfTaskResponseDTO> UpdateFarmToolsOfTasksStatusFinishAsync(string FarmToolsOfTasksId, string? note, int? quantity)
		{
			var farmToolsOfTaskId = Guid.Parse(FarmToolsOfTasksId);
			var farmToolsOfTask = await _unitOfWork.FarmToolsOfTaskRepository.GetByIdAsync(farmToolsOfTaskId);
			if (farmToolsOfTask == null) throw new KeyNotFoundException("FarmToolsOfTask not found.");
            ...
            if (note != null)
            {
                ...
                farmToolsOfTask.Status = 5;
                if(quantity > farmToolsOfTask.FarmToolOfTaskQuantity)
                {
                    throw new KeyNotFoundException($"Farm tool quantity used {farmToolsOfTask.FarmToolOfTaskQuantity}.");
                }
                farmToolsOfTask.FarmToolOfTaskQuantity = (int)(farmToolsOfTask.FarmToolOfTaskQuantity - quantity);
